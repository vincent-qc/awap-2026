...
class BotPlayer:
    ...
    def get_free_counter(self, controller, exclude=None, bot_pos=None, blocked_tiles=None):
        ...
        if bot_pos:
            counters = sorted(counters, key=lambda c: max(
                abs(c[0]-bot_pos[0]), abs(c[1]-bot_pos[1])))
                ...
                if holding.get('type') == 'Plate':
                    if not state.plate_counter:
                        state.plate_counter = self.get_free_counter(
                            controller, exclude=reserved_counters, bot_pos=(bx, by), blocked_tiles=blocked_tiles)
    ...
    def process_chop_only(self, controller, bot_id, state, holding, reserved, blocked):
        if not state.work_counter:
            ...
            state.work_counter = self.get_free_counter(controller, exclude, bot_pos=(
                bot_state['x'], bot_state['y']), blocked_tiles=blocked)
